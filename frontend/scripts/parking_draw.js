...
function startDrawing(grid) {
    ...
    */
	...
	const carHeight = carProportion * carWidth;
	const cellWidth = carWidth + 2 * freeSpace + lineWidth,
		cellHeight = carHeight + 2 * freeSpace + lineWidth;
			...
			for (let j = 0; j < cols; j++) {
				...
				switch (grid[i][j]) {
					...
					case gridEncoding.takenNormal:
						...
						context.drawImage(
							car,
							j * cellWidth + (cellWidth - carWidth) / 2,
							i * cellHeight + (cellHeight - carHeight) / 2,
							carWidth,
							carHeight
						);
						break;
				...
				//draws horizontal lines
				if (
					grid[i][j] != gridEncoding.road &&
					grid[i][j] != gridEncoding.entrance
				) {
					context.fillStyle = "black";
					if (i <= lastLineWithCars) {
						context.fillRect(
							j * cellWidth,
							i * cellHeight - lineWidth / 2,
							cellWidth,
							lineWidth
						);
					}
		...
		for (let i = 1; i < path.length; i++) {
			...
			if (path[i] != path[i - 1]) {
				...
				switch (path[i - 1]) {
					case pathEncoding.up:
						context.fillRect(
							col * cellWidth + (cellWidth - pathWidth) / 2,
							offsetRow + row * cellHeight,
							pathWidth,
							height
						);
						break;
					...
					case pathEncoding.right:
						context.fillRect(
							col * cellWidth,
							row * cellHeight + (cellHeight - pathWidth) / 2,
							width,
							pathWidth
						);
						break;
...
>>>>>>> aeffc566cf9997ce65b262e9c4cfedcfeb405373
            case pathEncoding.up:
                context.fillRect(col*cellWidth + (cellWidth - pathWidth) / 2, row*cellHeight, pathWidth, height);
                row --;
                break;
